...
// delete the shader and the program-
	if (pProgram)
		glDeleteProgram(pProgram);
// the deletion of a shader program detaches the shader objects
	for (int i = 0; i < VSShaderLib::COUNT_SHADER_TYPE; ++i) {
		if (pShader[i])
...
//	
//	assert(pAttributes.count(name) != 0);
...
void VSShaderLib::addAttributes()
{
	int count;
	GLsizei actualLen;
	GLint size;
	...
	int maxUniLength;
	glGetProgramiv(pProgram, GL_ACTIVE_ATTRIBUTES, &count);

	glGetProgramiv(pProgram, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &maxUniLength);
	...
	for (int i = 0; i < count; ++i) {
		...
		glGetActiveAttrib(pProgram, i, maxUniLength, &actualLen, &size, &type, name);
		// -1 indicates that is not an active uniform, although it may be present in a
		// uniform block
		loc = glGetAttribLocation(pProgram, name);
		if (loc != -1)
...
void VSShaderLib::addAttribute(std::string name, GLenum type, unsigned int size)
{
	myAttributes u;
	u.type = type;
	u.location =  glGetAttribLocation(pProgram, name.c_str());
	u.size = size;
	pAttributes[name] = u;
}
