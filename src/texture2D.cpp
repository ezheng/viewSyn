...
texture2D::texture2D(): _width(0), _height(0), _textureID(0)
{
}
...
{	
	...
	{			
		//create(pPixels);
		std::cout<< "Error: the size of the image changed" << std::endl;
	}

}

void texture2D::createRGBA(const GLubyte *pPixels)
{
	if(_textureID != 0)
		glDeleteTextures(1, &_textureID);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, _width, _height, 0, GL_BGRA , GL_UNSIGNED_BYTE, &pPixels[0]);	

	glBindTexture(GL_TEXTURE_2D, 0);
}

void texture2D::upLoadRGBA(const GLubyte *pPixels, int width, int height)
{	
	if(_width == width && _height == height && _textureID != 0)
	{		 
		glBindTexture(GL_TEXTURE_2D, _textureID);
		glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, _width, _height, GL_BGRA, GL_UNSIGNED_BYTE, &pPixels[0]);
		glBindTexture(GL_TEXTURE_2D, 0);
	}
	else
	{			
